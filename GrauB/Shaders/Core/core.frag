...
uniform float Ns;
vec3 light_position_world  = vec3 (1.0f, 1.0, 1.0);
...
void main()	{
	...
	vec3 light_position_eye = vec3 (ViewMatrix * vec4 (light_position_world, 3.0));
	...
	if (selecionado) {
	   FragColor = texture(texture1, TexCoord) * vec4(1.5,1.5,1.5,1.0) * vec4 (Id + Ia + Is, 1.0);
	}else{
	   FragColor = texture(texture1, TexCoord) * vec4(1.0,1.0,0.0,1.0) * vec4 (Id + Ia + Is, 1.0);
	}
}